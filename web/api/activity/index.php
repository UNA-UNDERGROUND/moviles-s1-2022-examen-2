...
try {
    ...
    if (
        ...
        // check whether the body is a valid Activity object
        if ($json == null) {
            throw new Exception("Invalid Activity object", 400);
        }
        $activity = Activity::fromArray($json);
    ...
    if ($verb == "OPTIONS") {
        ...
        $result = $controller->updateActivity($activity);
    } else if ($verb == "DELETE") {
        // check if there is a valid id
        if ($id == null) {
            throw new Exception("Missing Id", 409);
        }
        $activity = $controller->getById($id);
        $result = $controller->deleteActivity($activity);
    } else {
